...
## 2026-09-15：Pi 架构参考改造（覆盖下面历史批次进度）

参考 [Pi 固定版本装配合同](https://github.com/earendil-works/pi/blob/f9bcd351dc3cedf989bc5fc0f8aa012db5737df2/packages/ai/src/models.ts)，独立实现，不复制源码或增加外部服务。

- 供应商默认值、协议适配器注册和已有认证解析分别装配；协议切换不套用其他协议的 JSON 默认参数。
- Messages 不再无条件发送 Claude 专属 adaptive/结构化输出字段；显式设置仍消费，不支持的参数显式拒绝。
- 三协议共用工具历史请求投影，保证跨协议 ID 与结果配对、持久回执不变，缺失结果不补造。
- 原生流新增统一事件合同，包括工具参数增量和已验证终态；旧回调兼容，客户端逐字符参数预览未在本批新增。
- 关闭 SDK 隐式重试；目录发现可取消；新模型不自动继承旧模型省略的上下文窗口/价格。
- 保留当前 Grok OAuth、安全存储与实时目录查询；没有把 Pi 的内置 xAI 模型清单移植为“账号实时可用性”。

最终 21 文件 / 173 项通过，1 项真实连接测试跳过；构建和边界检查通过。实际账号/模型组合仍须独立验收，范围并非 Pi 全供应商、多模态及全部授权方式。详见开发状态文档同日批次；下面数字保留为历史，不作为当前验证基线。
