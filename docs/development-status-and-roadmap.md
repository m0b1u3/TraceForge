...
当前优先级覆盖下方历史批次的“下一步”：客户端配置闭环与对话/调查生命周期已有实现，继续推进 macOS 桌面的完整用户旅程，不把独立包或表单保存视作产品完成。Scenario 提示词、Skills、知识资源绑定及 MCP 配置仍须经过“编辑 → 保存 → 实际运行消费 → 可核对生效内容”，不得要求日常用户改源码或重打场景包。当前自动验收使用中性合成协议端点，不连接外部 CTF、不使用用户真实模型账号；真实模型/目标效果另行由用户手动验收。长期经验记忆仍是独立缺口；本轮本地工具限于受控 Run 目录，具体实现和边界见下，不用新增基础设施无限推迟桌面可用版本。
