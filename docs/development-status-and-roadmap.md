...
| 范围 | 完成度估算 | 说明 |
| --- | ---: | --- |
| 通用安全智能体底座 | 约 72% | 核心架构已经成型，缺少 Provider 控制面、完整恢复、生产观测与部分执行闭环 |
| 单机生产化能力 | 约 58% | 已具备持久化、门禁和 Execution Node，但安装升级、Provider 包治理和长稳测试未完成 |
...
### 4.1 Tool Provider 还缺少完整控制面

当前可以通过组合根注入外部 Provider，但还没有持久化 Manifest、包签名、信任根、安装启用、版本回滚、排空升级和重启恢复。Provider 健康状态主要保存在内存，TraceForge 重启后需要重新发现。
...
- 定义 Manifest Schema：Provider id、来源、版本、可执行入口、文件哈希、签名、能力、权限、资源、平台和协议版本。
- 建立 Provider Event Store 与查询投影，持久化 installed、enabled、draining、disabled、quarantined、failed 状态。
- 建立本地信任根和签名校验；未签名、哈希不符、降级版本和来源冲突默认拒绝。
- 提供安装、启用、禁用、刷新、排空升级、回滚和查看审计记录的控制面 API。
- TraceForge 重启时恢复已启用 Provider，未完成调用按幂等回执和检查点处理，不盲目重放。
...
- 当前工作树中的底座增强尚未提交，应在进入 Provider Manifest 开发前形成一个可回滚提交点。

## 7. 最近的明确开发目标

下一项应直接开发“持久化 Tool Provider Manifest 与供应链控制面”。这是动态工具运行时、热升级、重启恢复、第三方工具隔离和后续 Web 场景工具包的共同前置条件。完成它之前，不应继续堆叠新的具体漏洞工具。
