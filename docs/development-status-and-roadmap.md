...
依据用户要求，取消接入层的 4096 输出降额、角色默认累计/单次 token 配额、角色/对话默认总超时、对话 8 轮/6 次工具调用、Worker 默认 24 轮、长任务默认 240 轮/120 分钟、压缩/收尾默认超时，以及单条回复 64 KiB 截断。接入配置与模型能力声明原值传递；未声明时 Chat Completions/Responses 不添加输出参数，Anthropic 要求的字段缺失则说明配置不足，不制造默认值。未知窗口估算不作为请求拒绝依据。任务预算字段支持真正留空，现有授权快照不被软件更新扩大。

保留停止/撤权、沙箱/授权、回执与未知执行纪律、明确配置的预算及本机容量保护。详见 [限制审计](model-execution-limit-audit.md)，逐项区分删除与保留，不宣称把仓库所有数值限制盲删。界面按 Impeccable clarify 保留布局，仅同步参数来源和容量错误文案。

最终专项回归 **27 文件 / 285 项通过**，受影响集成回归另 **6 文件 / 55 项通过**；含超过旧轮数、工具数、回复长度和默认时间后继续运行，以及停止仍有效。最终全项目构建、底座边界（303 文件）、网络边界（16 文件）及差异空白检查通过。完整快速回归曾为 **2643 通过 / 6 失败 / 40 跳过**，六个失败已修正并在上述集成回归重跑通过；最终修改后未重复整套快速回归，不将分批结果冒充同一轮全绿。先前 DeepSeek 原生测试已暂停关闭，未完成的浏览器接管旅程仍如实保留；下一优先级是恢复同一隔离配置的真实接管、交回与停止清理验收，不再临时提高模型额度。

### 联合验收继续：能力与授权动作的模型上下文

真实 DeepSeek 在 macOS 原生桌面已越过 Scenario 加载和工具快照故障，实际执行范围读取及上下文工具；随后把能力名直接与 `authorizedActions` 比较，误报浏览器未授权。现将已固定 Scenario 的能力→动作映射传给 Planner 和 Worker，Worker 同时收到实际 allowed/denied 动作；明确 Work 局部目录缺项不等于 Run 全局能力不可用。映射只说明合同，不授予权限、不证明宿主就绪，实际派发校验不变，无黑盒专用 Core 分支。

新增权限投影与动态任务表单回归，专项 **3 文件 / 19 项通过**，全项目构建通过。原生截图确认授权表单已自动显示，先前“必须切换页面”的判断来自未更新的辅助功能树；未据此修改刷新业务逻辑。第一轮已停止；修复后的第二轮真实模型旅程继续中，**尚不宣称浏览器接管与交回通过**。下一验收仍是同一浏览器会话内接管、输入、交回读取与停止清理，不增加远程节点或更多权限。

### 当前增量：任务动作收窄与 macOS 场景进程准入

执行原则按用户本轮要求收敛：优先保证既有沙箱、宿主文件隔离、任务目标范围及停止/撤权有效；在明确授权的范围内连续执行，不把每条常规命令变成新的审批仪式。不新增远程执行或另一套权限机制。动作选择是一次任务的范围选择，不是逐次工具确认；危险操作与越界仍走既有明确授权。

- 黑盒包 **0.5.15** 启用通用 `actionSelection` 合同。授权页可逐项选择操作，未选择即拒绝；选项、资源范围、有效期在同一复核中确认，修改后清除旧确认。新表单默认未选择，不暗中授予工作目录、安装或命令权限。SDK 拒绝缺失/未知/重复动作及占用保留字段的表单；选择进入既有授权快照与逐次服务校验，重启不丢失。暂停后变更沿用既有修订与撤权路径，无第二套权限账本。
- 对话的任务目录投影 Scenario 声明的能力→授权动作对应关系，避免模型仅因名字不同就误判“未授权”；声明不等于实际工具已加载，不赋予执行权限。
- 原生隔离测试确认了另一处装配缺口：场景进程客户端仍强制要求硬资源配额，导致实际 macOS 监测终止模式无法加载场景工具。现沿用 Browser Runtime 同类校验：仅宿主明确接受 `sampled_terminate`、固定后端及测量值且返回进程树归属/清理屏障证明时通过；默认硬配额路径不变，不把监测终止谎称硬配额，不放宽文件或网络沙箱。
- 隔离测试启动器补齐原生 helper/release 清单及显式 standalone Node 读取许可；不再把 Electron 主程序当 Node 启动场景。测试场景自身断网，仅通过已有宿主能力操作已授权页面，不改变用户日常模型配置。
- 原生启动继续暴露并修复两个默认值冲突：Provider 未指定输出额度时从 Node 握手的实际容量收敛默认值（显式配置不静默修改）；macOS 不再额外拒绝超过一小时的宿主时限，仅保留 JS 定时器可表达范围，既有任务时限、租约、停止与监督不变。真实 Electron 中场景源已达到 `ready`，成功加载 15 个工具，不是依靠测试替身宣称可用。
- 场景源实际启动后，真实模型任务揭示工具目录展开把内部服务对象/回调带进上下文，触发 `could not be cloned`。Gateway catalog 与 discovery snapshot 改用已有 `snapshotToolSpec` 的白名单投影，不另建摘要设施；回归覆盖不可克隆回调、内部值不暴露及正常合同可克隆。

实际验收：真实 DeepSeek 通过正常桌面对话准备任务，UI 显示并保存 **5 项**明确授权；未选择的工作目录/脚本/安装等动作不包含在授权内，随后成功启动并停止任务。首次实测发现原生组件未配置及硬配额准入残留，已针对性修复。**浏览器打开→人工接管→交回的完整旅程尚未因这次入口通过而宣称完成**；测试中的模型仍有将声明误认为就绪、根据不完整记录误判授权的现象。

验证：完整快速回归 **307 文件（303 通过、4 条件跳过），2672 项（2633 通过、39 条件跳过），零失败**；全项目构建通过。随后补入输出额度协商与原生时限修正，Provider 专项 **19 项通过**、真实 macOS 沙箱专项 **20 项通过**，工具目录/发现专项 **40 项通过**，Worker/Execution Node 构建通过；真实测试只验证一天时限配置准入和正常退出，未跑满一天。原生真实模型验收最终在工具快照错误处停止；修复已通过回归，但尚未再次跑完接管/交回，不能声称完成。隔离实例已关闭，记录保留，用户日常配置未改动。下一明确优先级是验证修复后的本地中性页面联合流程，并收敛重复询问与复杂授权呈现，而不是继续增加限制或设施。

收尾增量：最终代码的 Worker/工具目录/Provider/场景装配/任务端口联合回归 **8 文件、114 项通过**；全项目重新构建通过，保留 Web 分块体积警告。Node 数据库依赖恢复，`git diff --check` 通过。授权 UI 按 Impeccable harden 沿用现有复核，不增加新的审批页面；原生截图及实际勾选/返回/提交已经检查，默认全未选，未选权限没有被自动补齐。本批未提交推送。

### 当前验收：真实 DeepSeek 与原生桌面任务入口

本轮仅使用官方 DeepSeek `deepseek-flash`，隔离测试配置，不修改用户日常模型配置。原生启动器支持从标准输入接收配置，用 Electron secure storage 加密保存密钥；启动及恢复拒绝其他供应商/备用路由。修正启动器等待 Electron ready 的顶层等待死锁；本地中性网页夹具不访问外部目标，重启复用原端口，不静默切换授权目标。

- **真实模型自动化通过**：当前消息/记忆、结果驱动规划、流式停止与重启三项联合测试通过，24 次实际 POST、报告用量 34218 tokens，6 次无用量返回。部分结构化输出请求先被供应商拒绝，再经既有兼容路径成功；不隐藏这些请求。规划测试的空转决策仍为脚本替身，其余规划/工具/重规划/确认使用真实模型。这不是漏洞发现效果验证。报告：`data/desktop-model-acceptance/traceforge-model-batch-axSm3N/report.json`。
- **修复任务目录信息丢失**：`task_context` 原来仅投影名称/简介，现提供通用的必需能力、Worker 能力、阶段目标及授权动作，明确只是声明，不是授权或运行时就绪证明。工具说明禁止按名称臆测模式、把动作清单误认为必做操作，或捏造界面不存在的授权选择器。新增测试覆盖投影和只读边界，不包含黑盒专属判断。
- **原生桌面实测结果（部分通过）**：隔离 Electron 使用真实 DeepSeek，经正常对话工具循环、消息持久化和重启恢复；明确指定已安装场景及受限观察意图后，模型生成 `task_request`，客户端显示范围复核。没有提交授权、没有启动 Run，浏览器打开/人工接管/交回/停止联合流程尚未验收。
- **实际暴露的剩余缺口**：自然语言浏览器验收请求仍被模型误判为不匹配；能力名称不足以解释 open/observe/人工接管，需来自实际工具合同的通用说明投影，不能往底座写黑盒提示词。当前授权表单列出固定动作，不提供模型声称的逐项选择；只能填资源范围，不把单页 URL 等同动作已收窄。未接受宽于测试意图的授权，也未绕过 UI 登记授权。

最终本轮增量验证：Server 构建通过，任务端口/桌面回复/桌面执行路由 **3 文件、45 项通过**；启动脚本语法检查通过。此前完整回归仍为下方记录，本轮未重跑全量，不将旧结果充当新版本全量结果。隔离实例已关闭，Node 数据库依赖已恢复；未提交推送，远端 CI 仍未执行。

下一明确优先级：补齐模型可读的实际工具用途与授权动作收窄，再在用户确认的本地中性测试范围内完成原生浏览器联合验收。通俗地说，让模型知道“已经能做什么”，让用户真正只授权“这次要做的事”，再证明同窗接管、交回和停止可用；不能把当前入口通过说成全链通过。

### 当前整改：统一目的地检查、回执来源与架构回归门禁

**2026-09-21 收尾补齐**：MCP 新增客户端目标 IP 绑定，校验 IPv4/IPv6，保留域名 TLS 身份，解析结果全量受绑定约束；配置保存、测试、启用与旧 Run 修订隔离均经过测试。工具回执、流量、附件的图来源统一走只读来源投影，全部校验同 Case/Run，取原始时间/生产者和摘要；未完成流量拒绝。Artifact 摘要来自既有内容合同，不能将只读投影视为已经重新读取所有外部内容。HTTP 实际目的地补入持久回执，保存时核对所属任务与请求/响应身份，事务保证流量及回执同时保存或同时拒绝。既有数据库通过新增可空字段升级，未记录的目的地不伪造。

本轮整改范围已完成：修复已发现的网络/来源绑定缺陷，收拢网络机制与证据来源职责，验证非 Web 复用并建立 CI 门禁；不将“把 Server 所有模块搬走”或“必须新增正式 Scenario”设为无边界的完成条件。远端 CI 执行需推送后发生，真实模型桌面验收属于另一个验收范围，不冒充本次已执行结果。

**收尾后的最终同版本基线**：全项目构建、Server foundation 类型检查通过；完整快速回归 **307 文件（303 通过、4 条件跳过），2662 项（2623 通过、39 条件跳过），零失败**。退役代码、303 个生产文件的底座边界、16 个入口/Scenario 网络门禁和 13 个可重现 runtime 模块检查通过；最终 `git diff --check` 通过。MCP 设置使用 Impeccable harden 保留现有样式并补字段校验，CUA 在隔离合成页确认错误显示、长 IPv6 输入与保存只产生草稿修订；没有改用户实际配置。未调用模型，未提交推送。

针对架构审查中可坐实的问题实施本批整改，不新增远程节点、多用户、正式白盒产品或第二套执行账本。

- **网络连接边界**：在 Execution Node 集中实现解析、全量 A/AAAA 检查、解析后再授权和固定地址连接。默认 HTTP Broker、Workspace HTTP/隧道/WebSocket、公共资源读取及 HTTP MCP 使用相同机制；默认 Broker 不再直接 `fetch`。非公网 DNS 结果要求字面 IP 范围授权，公共资源仍仅允许公网。保留逻辑 Host/TLS 身份、逐跳授权、不自动重试及回执目的地记录。具体影响和限制见 [网络目的地边界](network-destination-boundary.md)。
- **证据来源**：工具结果引用改为读取既有 binding/receipt 的统一只读投影，要求同 Case/Run、调用已完成、结果成功；来源时间和生产者取原始回执/绑定，内容生成 SHA-256，冷归档读取沿用摘要校验。成功工具结果仍不等于已验证漏洞。未把 traffic、artifact、模型总结混为同一种来源，也未宣称所有历史数据都具有同等来源强度。
- **职责拆分**：DNS、HTTP、TCP 和 WebSocket 建连机制移至通用 Execution Node；Server 保留授权、任务存活检查和数据库装配。不按文件大小大规模搬迁，也不宣称 Server 已完全轻量化。
- **非 Web 验证**：新增中性文件 Scenario 集成夹具，经真实 Host → Worker → Gateway → 工作区写/读 → 持久回执完成任务，无 Web capability、无 Core 专用分支。模型决策为确定性替身，macOS ARM64 条件运行；这是复用验证，不是第二个正式发货 Scenario。
- **CI**：新增 GitHub Actions 的 macOS/Linux Node 22 构建、快速回归、退役代码/底座/网络边界及 Scenario 可重现构建门禁。未推送，不能声称远端 CI 已通过；也不把 Linux CI 等同 Linux 原生沙箱验收。

验证：全项目构建通过（保留 Web 500 kB 分块警告）；完整快速回归 **306 文件：302 通过、4 条件跳过；2657 项：2618 通过、39 条件跳过，零失败**。网络/资源/Workspace/MCP/证据专项首轮 59 通过、3 条件跳过，新增非 Web 集成通过，真实本地 HTTP/SSE 与归档专项 9 通过。底座 303 个生产文件、网络 16 个入口/Scenario 文件及 13 个 Scenario runtime 可重现模块检查通过；`git diff --check` 通过。本批不调用真实模型，未提交推送。

剩余产品验收边界：没有开发第二个正式 Scenario（已完成中性文件场景复用验证）；没有调用真实模型或执行完整原生桌面联合验收，不声称所有网络协议透明兼容。MCP 私有域名地址绑定缺口已在上方收尾补齐。下一明确优先级是隔离 DeepSeek 配置下的完整桌面联合验收，确认正常对话入口的授权、浏览器人工接管、交回继续与停止实际可用，而非继续拆出无用户结果的小设施。

### 当前整改：浏览器目录准入、人工接管等待与关闭失败恢复

本批针对 `91edfaa` 审查发现的实际装配及生命周期缺口修复，不新增执行器，不扩展到报告、发行、远程节点或多用户平台。

- **浏览器目录准入**：新增通用 `browser-host` 权限配置。Scenario 声明能力和授权动作；宿主核对浏览器部署、Execution Node、可用包的来源/版本及 Browser Host capability。只有匹配工具取得受限 `sandboxed + brokered` 配置，文件路径仍为空、交互终端/后台执行不开放；无宿主、撤权及不匹配来源继续拒绝。黑盒包更新为 **0.5.14**，不修改已签名安装包或旧 Run 的快照，已安装版本需正常审核激活新包才能使用新声明。
- **人工接管等待**：Worker 增加通用宿主等待接口，由 Server 的浏览器会话状态驱动；接管正在提交或人工控制期间，后续模型评估、工具派发及完成/阻塞/审批/耗尽分支等待交回，同时保持续租。接管期间形成的模型决定及持久化阶段被接管打断的完成/阻塞意图重新评估。已派发的操作不会假装撤销；停止、撤权、会话期限及长任务授权/总时限仍生效。Scenario 与 Core 不包含 Electron 或黑盒专用等待逻辑。
- **关闭失败恢复**：清理成功后才删除活会话；失败保留句柄和 `cleanup_unknown` 状态，禁用继续操作，允许撤权后的显式清理重试。并发清理合并，成功后的重复关闭无新副作用；首次清理失败不提前终结容量凭据。关闭期间迟到的接管/交回不能恢复关闭状态，原生页面也不会重新获得人工控制。
- **桌面恢复入口**：沿用既有折叠区，显示“关闭尚未确认 / 重试关闭”和“由你控制，智能体等待交回”；不自动重放点击、输入或失败关闭。按 Impeccable 的异常恢复与收尾规范处理，未新增卡片或改版。组件测试与 CUA 合成状态交互已核对；这不是完整原生桌面任务验收。

验证：最终专项 **8 文件 / 99 项通过、1 项真实 Chromium 条件跳过**。含发货 Scenario 声明 → 实际 Tool Gateway 目录/权限 → Scenario 子进程 → Browser Runtime → Broker 回执/观察记录的集成测试，不再给该测试直接注入有效浏览器权限；浏览器控制器、网络响应和模型决策仍为测试替身。全量快速回归实际执行 **303 文件 / 2632 项**，结果为 **2592 通过、39 条件跳过、1 失败**；唯一失败为旧测试仍断言独立浏览器打包，与上一批已切换的 Electron 内嵌装配不符。本批将断言改为检查实际内嵌装配、Chromium 沙箱/无 Node、无独立浏览器资源及保留发行门禁，该文件 **5 项通过**，纳入上述最终专项；未把未再次执行的全量宣称为零失败。全项目构建通过，最终 Server/Worker Runtime 类型检查通过；Web 仍有大于 500 kB 分块警告。底座边界 **299 个生产文件通过**；`git diff --check` 通过。本批未调用真实模型、未提交推送。

明确边界：宿主重启后不恢复已丢失的浏览器活句柄，不自动重开页面；未知清理记录不等同已确认清理。未运行真实模型或重新进行完整原生窗口任务验收，不把本批组件/子进程测试称为用户正常发起任务的全链通过。下一明确优先级仍是使用隔离 DeepSeek 配置，在完整 HostWorkbench 中联合验收“新对话授权 → 打开网页 → 人工接管 → 交回继续 → 停止”，并核对新版本包激活。通俗地说，要确认用户从正常对话入口也能完整走通，而不只是单个部件可用。

### 当前增量：同一桌面窗口内的原生任务浏览器

桌面默认装配改为 Electron 自带 Chromium 的 `WebContentsView`，不调用用户 Chrome，也不再装载上一阶段的独立 Headless Shell。右侧显示真实网页，左侧保留对话；不再使用截图轮询或坐标转发模拟网页。以下 Headless Shell/CDP pipe/资源签名记录属于前一阶段独立浏览器路径，不代表当前桌面装配。

- 原生实现仅在 Desktop；Server 接收通用部署工厂并装配原有 Browser Runtime、持久证据存储、授权和接管生命周期。Scenario 不知道 Electron，不引入第二套任务执行器。启动应用只装配端口，不擅自创建网页或发起任务。
- 接管成功自动展开页面，支持直接输入、粘贴、滚动和文字选择；收起仅隐藏，交回后继续操作同一页面。宽窗为左右布局，窄窗覆盖内容区并保留导航。连接失败明确显示恢复入口，不重放网页动作；应用弹窗出现时隐藏原生子视图，展示心跳消失后自动收起。
- 原生页使用独立临时会话、Chromium 沙箱和上下文隔离，无 Node、preload、桌面 IPC；拒绝新窗口、页面权限和磁盘下载。桌面 IPC 核对本窗口主 frame、所属对话/Run 与当前接管标识，再允许展示；越界区域、旧接管和非人工控制拒绝显示。键盘“进入网页”及 F6 返回应用、卸载焦点恢复已补齐。
- CDP 仅附着该 guest 与其子目标，不全局发现或接管桌面窗口；控制证明准确标记 `electron_debugger`。HTTP 仍通过现有逐请求授权和 Broker 回执；未被截获的 HTTP 采用不可达代理阻断。保持用户选择的 Chromium-only，不加外层 macOS 包装器，不把它描述成内核拒绝联网、通用硬配额或完整任意协议兼容。版本散列仅为构建身份，不冒充浏览器二进制测量或独立供应链审计。
- 会话关闭、交回和停止先撤走显示；清理等待 guest 销毁并清除会话存储，重复清理合并，失败不冒充成功。桌面退出也执行兜底清理。打包清单不再携带已不使用的独立浏览器树；现有本地材料保留作独立路径回归，不删除用户浏览器数据。

验证：浏览器核心、桌面组件、Server 装配与宿主专项合计 **9 文件 / 86 项通过、1 项旧真实 Chrome 条件跳过**。本机 Electron 原生验收通过：真实 WebContentsView、内层沙箱、同页交回、一次性准备对象、失效接管/越界拒绝、展示心跳过期与销毁清理。CUA 同窗操作验证输入、粘贴、滚动、键盘进入/F6 返回及交回后智能体读取同一输入，网页中 Node/桌面接口为 undefined。Server、Desktop、Browser Runtime TypeScript 与 Web 构建通过；Web 保留大于 500 kB 的构建分块警告。底座边界 **299 个生产文件通过**；`git diff --check` 通过。本轮不调用模型、不提交推送。

验收边界：原生窗口使用实际产品 BrowserViewport 与 Native Manager，但左侧为合成会话；不等同完整 HostWorkbench 真实任务旅程。`capturePage()` 只拍到宿主，输出明确命名为 host-only，不能用于声称网页可见；实际组合画面由 CUA 检查。按 Impeccable 完成既有界面的收尾复核与 surface 记录。扫码、WebAuthn、完整多标签、任意协议、签名公证及 Electron 安全版本维护仍未在本轮验收，公开发行门禁不解除。

下一明确优先级：在完整桌面对话任务中联合验收“打开网页 → 人工接管 → 交回 → 停止”。通俗地说，核对用户正常发起的任务也能使用这块真实网页，而不只是在验收窗口中可用；需要真实模型时仅使用隔离的 DeepSeek 测试配置。

### 当前变更：桌面浏览器仅保留 Chromium 自身沙箱

按用户明确选择，macOS Apple Silicon 桌面自带浏览器现固定采用 `isolation: chromium`：保留 Chromium 默认沙箱，不调用外层 macOS 执行启动器，也不添加 `--no-sandbox`。这是浏览器专用宿主装配选择，不是启动失败后的回退，不改变终端、脚本、MCP 或通用 Execution Node 的隔离要求。

- 新增浏览器专用生命周期端口。正式安装验证发行材料、浏览器树和可执行摘要后，仅允许消费本安装准备的一次性启动对象，不能经 Scenario/客户端传入任意程序路径。独立 profile、CDP pipe、期限终止和进程组清理继续保留，应用启动优先使用已准备的自身资源，不搜索本机 Chrome。
- 继续复用任务归属、容量占用、逐请求授权、HTTP Broker 与回执、证据存储、人工接管/交回纪律。截图和坐标输入在同一浏览器进程内生效。会话明确标注 Chromium 隔离，Controller 网络证明改为 `application_intercepted`；不伪造外层 OS 拒绝联网证明。
- 明确边界：Chromium 的特权主进程没有外层任务文件隔离；CDP 授权拦截不是系统防火墙，不宣称覆盖一切浏览器内部协议或浏览器漏洞后的行为。CPU/内存外层额度不在此模式执行；保留宿主期限和生命周期控制。应用内 CDP 适配器不冒充已运行独立 Controller 可执行文件。
- 修正关闭状态：先冻结并清理，确认终止后才显示 closed；清理不确定时不删除 scratch、不释放占用为成功，允许后续核对。来源采纳说明与最终应用资源已按本次决策重新准备，30 天材料信任期限仍存在，不引入自动更新。

验证：专项 **13 文件 / 82 项通过、1 项旧真实 Chrome 条件跳过**；最终应用资源的真实 macOS Chromium-only 用例 **1 项通过**，实际检查启动参数不含关闭内层沙箱或外层包装器，验证安装签名/摘要、页面经 Broker 访问、截图、人工点击/输入、交回后读取结果、关闭清理及到期停止。Browser Runtime、Server、Desktop TypeScript 构建通过，底座边界 **299 个生产源码文件通过**，`git diff --check` 通过。没有调用模型、没有提交推送。

该原生验证是实际宿主/浏览器链路，不是原生桌面窗口的人工视觉验收。完整内置浏览器任务仍待桌面视口视觉与原生窗口交互联合验收；扫码、WebAuthn、完整多标签、签名公证与公开发行未验收。下一优先级是确认用户在桌面一个窗口内能顺畅接管和交回，而不是继续更换沙箱方案。

### 当前产品纠正：应用自带浏览器，不依赖用户 Chrome（未完成）

以下为切换隔离方案前的阶段记录；其中材料未安装、等待沙箱决策的状态已由上节取代，历史失败保留作为诊断依据。新 Headless Shell 已准备到默认应用资源目录。

用户明确要求应用内真实浏览器。此前持续会话及元素面板只是基础能力，不满足这个产品目标，真实本机 Chrome 测试也不能替代它。当前任务尚未完成，不把以下装配改动描述为完整内置浏览器。

- 已新增桌面自带浏览器材料装配：正式应用只从自身 resources/browser-runtime 解析固定的浏览器发行树、独立信任材料和 Node 运行时；环境变量指定安装路径仅保留在开发启动。缺材料、平台不符、额外路径字段、软链接逃逸及 scratch 重定向明确拒绝，不搜索用户 Chrome。
- 打包资源清单已纳入该目录，执行前仍经过原宿主完整性与来源验证，不以打包路径替代授权。资源目录格式记录在 apps/desktop/runtime/README.md，二进制与签名材料不混入源码。发行门禁没有解除。
- 已按用户批准准备官方 Chrome Headless Shell 153.0.8010.52（macOS ARM64）候选材料。新增独立上游二进制采纳合同，明确不是可复现源码构建或独立安全审计；绑定官方地址、下载摘要、树摘要、采纳说明及应用独立公钥。准备脚本签名私钥仅驻内存，候选授权有效期 30 天；不增加自动更新。此前完整 Chromium 1701457 在应用注册阶段崩溃，已移出默认资源目录，保留为失败诊断材料。未将候选安装为默认可用浏览器。
- 已写入桌面像素画面和人工输入链路：受控进程输出 PNG，宿主绑定当前接管与短期画面编号，支持坐标点击、文本、按键与滚动；远程 HTML 不进入应用 DOM。预览不自动作为证据保存，输入正文不写命令账本，未知动作不重放。当前是轮询图片而非原生嵌入窗口或视频流；尚未完成本批界面视觉验收及原生产品联合验收，不宣称完成。
- **实际阻塞（2026-09-20 复核）**：旧原生集成测试启动器暗中附加 `--no-sandbox`，只验证外层 Seatbelt。该模式使用新引擎 1 项通过，但不能证明产品默认启动可用。本批纠正测试：默认使用产品沙箱参数，仅显式 `TRACEFORGE_TEST_MACOS_BROWSER_OUTER_ONLY=1` 才运行标明非产品验收的历史诊断模式。默认参数真实测试 **1 项失败**（首个 DOM 无法取得、CDP Adapter 失活）；独立隔离诊断同时出现内层 sandbox initialization failed / GPU 进程崩溃，取消清理通过。产品未关闭 Chromium 内层沙箱、未放宽外层权限。
- 下一优先级仍是同一内置浏览器产品任务：先解决内外沙箱启动兼容，再贯通安装授权与“页面 → 接管 → 交回 → 停止”的桌面验收。仅采用外层沙箱会改变浏览器漏洞后的隔离层次，必须明确决策，不能以测试启动器偷偷代替产品方案。源材料准备和组件测试通过均不能替代这一门槛。

本增量此前验证：核心、发行装配、宿主及界面专项 **12 文件 / 78 项通过、1 项条件跳过**；Browser Runtime、Server、Web、Desktop 构建通过，底座边界 **298 个生产文件通过**。真实原生产品启动失败如上，不能合并为全部通过。本轮未调用模型、未提交推送；新视口视觉检查与安装授权联合链路仍未验收。

最终来源合同与装配复验 **4 文件 / 13 项通过**，原生用例在默认测试环境条件跳过（显式原生运行失败已单列）；`git diff --check` 通过。移除本次准备旧候选时生成的临时签名私钥，保留公开验证材料；不涉及用户模型凭证。Impeccable 要求的新视口视觉复核尚未执行，不给出界面完成结论。

### 当前增量：持续受控浏览器与桌面接管

同一 Work 内的浏览器现可跨工具调用保留：Scenario 的既有浏览器工具增加 open/observe/act/close，通用宿主管理句柄与生命周期，桌面对话任务提供接管、读取页面元素、交回智能体和关闭入口。inspect 保持一次性关闭。用户接管时智能体动作被拒绝，交回后必须重新观察并取得新元素引用；不新增第二套任务执行器，不将黑盒策略写入底座。

- 会话绑定包、Case、Run、Work、Worker、租约与授权身份；同一有效租约可以更新到期时间，但不延长会话固定截止。失去租约、Run 停止、包撤销和宿主关闭触发清理。启动后不恢复进程句柄、不重放未知动作；SQLite 保留操作状态与中断记录。
- 人工输入只发送到选定页面元素，不写入对话和命令审计正文；界面不渲染远程 HTML、不暴露任意 JavaScript。页面内容不可信，接管不扩大原授权或网络范围。重复命令只复用结果，不再次执行动作。
- 真实 Chrome 测试发现并修复首个页面尚未就绪时的观察竞态，以及 Controller 与宿主请求超时不一致。关闭时等待已开始的清理，避免退出遗漏正在回收的会话。
- 按 Impeccable 完成组件正常态、窄窗、异常和空态检查；折叠状态也显示“需要核对状态”，无可操作元素时显示明确解释。截图使用模拟数据，仅证明组件表现，不冒充原生产品验收。

验证：核心及桌面专项 **7 文件 / 63 项通过、1 项条件跳过**；单独提供 Chrome 环境后，真实浏览器接管专项 **1 项通过**，实际人工动作改变页面状态，交回后同一进程的智能体观察读到该状态。Scenario/宿主联合回归 **8 文件 / 93 项通过**。Server、Desktop、Web、Browser Runtime 与 Controller 构建通过，底座边界检查 **297 个生产源码文件通过**。本轮未调用真实模型，未提交推送。

**支持边界与下一优先级**：当前桌面接管是页面元素操作，不是完整可视浏览器窗口；扫码、验证码、WebAuthn、多标签完整交互尚未验收。会话限于同一 Work/租约，默认 5 分钟、最多 15 分钟，并受进程截止及请求/动作预算约束，不是跨重启无限会话。真实 Chrome 测试使用模拟执行节点证明，不能替代 macOS Seatbelt 与正式受信浏览器安装的联合验收；实际运行仍要求既有受信安装可用，不自动使用本机未审核浏览器。下一明确优先级是在 macOS 原生客户端完成“授权 → 持续浏览器 → 接管 → 交回 → 证据 → 停止/重开”联合验收，实际作用是确认上述能力在用户的一个窗口中可用，而非只有测试接口能用。正式报告、发行及 Linux 真机仍暂缓。

### 当前增量：桌面原始记录回读与工具回执入口

桌面既有 evidence/read 与证据侧栏现接入通用组合读取器，除已绑定 Browser 工件外，支持 `network-receipt:`、`traffic:`、`knowledge-node:` 和 `tool-receipt:`（也接受精确工具回执键）。引用只作数据库身份，不转文件路径或远程 URL。网络/流量/工具记录必须匹配当前会话 Case 与请求 Run；Case 共享知识节点额外核实 Run 归属。Server 主装配已替换，未增加第二套界面或 Scenario 专属底座逻辑。

- 工具完成活动仅在宿主确认持久化回执及其归属存在时加入可展开回执引用，不把返回文本冒充原始记录；该入口是桌面展示投影，不修改模型证据合同。
- 工具回执复用现有冷归档完整性读取，分页重建后的摘要保持一致。知识节点保留状态、版本、失效原因和来源；查看记录不改变验证状态。流量正文明确只是保存的内容、不保证完整线上响应，省略可能含凭据的请求/响应头；网络回执保留截断、授权引用等字段。
- 所有类型最多 4 MiB、每页 64 KiB；后续页必须携带内容摘要，内容变化拒绝拼接，Browser 读取同步采用该规则。超过总量上限仍拒绝，不宣称已支持任意大文件。UI 使用现有惰性文本/PNG 预览，不执行记录中的 HTML。

验证：Server 构建通过，联合回归 **6 文件 / 40 项通过**（宿主、证据接口、工具装配、Browser Host、侧栏客户端及组件）。覆盖未绑定回执拒绝、归属隔离、正文分页、内容变化、头信息省略、知识失效状态和冷归档回读。首次宿主测试受环境禁止监听临时端点影响；获准后重跑通过。本轮未调用真实模型，未重新执行原生 Electron 整窗验收，未提交推送。

**该批历史缺口已更新**：持续会话与桌面接管装配由上方 2026-09-20 增量补齐；其支持范围和未完成的原生联合验收以上方为准。宿主历史挂起的全部根因仍未宣称解决；正式报告与发行仍暂缓。

最终补充 Browser 后续页摘要必填后，重新构建通过，证据接口/客户端/组件 **3 文件 / 15 项通过**，`git diff --check` 通过。

### 当前修复：MCP 激活事务、工作区容量恢复、检索上界与实时健康状态

根据 `16df3fc` 审查建议修复四项已确认缺陷，不增加旧业务数据迁移或双版本兼容路径。运行中的 Run 修订与操作记录仍属于当前执行纪律，不能因“不需要旧业务兼容”而丢弃。

- **MCP 失败不连带停用正常版本**：待启用修订仅临时进入发现/装配，Runtime 激活成功后才公布 active 指针。失败撤掉临时来源，旧 active 与已绑定 Run 保持可用；激活期间新建 Run 仍固定已成功启用的版本。来源目录仅包含当前 active、已有 Run 实际绑定的修订及本次临时激活，不把失败审查记录自动当作可运行来源。故障注入覆盖替换失败、旧 Run 保持可用及激活期间创建 Run。
- **容量超限可受控恢复**：`workspace_list(recovery=true)` 返回最多 512 条的部分清单和截断标记；`workspace_read(metadataOnly=true)` 不加载正文，返回普通文件/目录身份与大小。`workspace_remove` 可使用刚检查的 `expectedIdentity` 删除一个普通文件或空目录，已有内容摘要条件仍可使用，但两个条件不能混用；不是递归删除。精确文件读取、检查与清理不再依赖整个目录先满足容量上限。新写入和执行仍保留容量守卫；租约、路径、读写授权、链接检查和未确认进程清理标记不绕过。身份改变、非空目录及不确定进程仍拒绝清理。大文件元数据可读不等于已实现大文本分块读取。
- **记忆显式上界独立执行**：队列改序导致序号较后的已完成回复仍按原可见性规则提供，但用户传入 `before` 后，必须同时满足该上界，不再被 `completed` 分支绕过。回归包含“已完成后序消息默认可见、显式上界排除、空区间”。
- **健康状态实时读取**：`/api/health` 使用 `llmService.hasProvider()`，不再返回启动时缓存的布尔变量。无模型启动后保存模拟配置的接口回归确认状态改变，不访问供应商、不返回密钥。

验证：联合回归 **7 文件 / 87 项通过**，Server TypeScript 构建和 `git diff --check` 通过。新增容量恢复覆盖 17 MiB 二进制文件、超过 512 条目录项、身份变化、符号链接、非空目录、空目录和忙标记拒绝；测试删除的只是各自隔离夹具数据，没有清理用户工作目录。MCP 测试使用模拟传输，模型健康测试仅保存虚拟凭据，未调用真实模型、未消费任何供应商额度。未提交或推送。

本轮没有宣称解决历史宿主挂起的全部根因，也没有把一次性 Browser inspect 改称人工登录接管。该批留下的持续 Browser Session/桌面接管装配由上方 2026-09-20 增量补齐；通用原始证据读取按上方增量所列记录类型提供，不包括任意文件或任意协议。正式发现报告与发行继续按用户之前要求暂缓。后续产品工作仍以原生桌面联合旅程为目标，不扩建远程节点或旧业务兼容层。

### 当前批次：对话任务回读与真实模型规划衔接修正

- **老任务仍可读取和补充**：桌面任务目录保留最近 20 个 Run 的显示上限，但 `task_read` / `task_input` 不再拿这个显示窗口作为任务存在性判断。显式任务引用通过受控宿主按 Run ID 精确查询，复核返回 ID 与会话 Case 归属；未知、跨会话引用拒绝。新增 21 个 Run 的回归，验证窗口之外的任务回读、补充说明、适配器重建后不重复提交。未增加新执行器，不改 Scenario 的授权与状态机。
- **Planner 不再需要猜证据引用类型**：真实 DeepSeek 记录显示，同一阶段连续六份规划把 `work:<id>` 或 Work ID 放入 `evidenceRefs`，被既有证据校验拒绝后重复推理。现模型输入提供来自当前可见上下文的精确 `referenceCatalog.evidenceRefs`，说明 Work ID/结果摘要/事件 ID 不能充当证据；不需要引用时填空列表。目录在正文压缩后保持精确身份，宿主仍拒绝伪造或未知引用，不把工作进度提升为验证证据。该修正为通用 Planner 合同，不包含黑盒目标、漏洞或供应商特例。
- **真实验收入口纠正**：增加 `--journey-only` 联合检查当前消息记忆、结果驱动规划、流式停止与数据库重开。相对输出目录曾导致本地进程宿主在调用模型前拒绝启动，现入口转换为绝对目录。入口只接受显式官方 HTTPS DeepSeek 端点，不修改用户 Grok 配置，不存凭据。用户要求本次排查不限制最大轮数，联合套件及其内部探针均支持无调用次数上限；单次请求/套件截止、停止及不重放规则继续保留，其他自动回归默认仍有界。

**实测而非推断**：DeepSeek Flash 最终三项联合验收通过，16 次模型 POST，约 33 秒，报告 `data/desktop-model-acceptance/traceforge-model-batch-IcdbnL/report.json`。规划阶段真实调用 8 次，形成 2 项顺序 Work、1 次原始工具动作、2 份回执，结果依赖可追溯，数据库完整性正常；修正前无次数上限对照为 14 次，其中重规划 7 次，修正后重规划 1 次。模型有随机性，不能把一次改善写成永不重试或固定性能保证。早期失败报告保留，不与最终结果混写；全部真实调用只用 DeepSeek。

验证：最终联合回归 **13 文件 / 115 项通过**，覆盖场景子进程/HTTP 续跑、配置实际消费、MCP 管理与激活、对话记忆、任务衔接、流式停止及 Planner 引用纪律；Server TypeScript 构建、`git diff --check` 通过。本次没有修改 renderer、没有重新执行原生 Electron 窗口完整用户旅程，没有把 API/SQLite 验收说成原生界面验收。

**剩余与下一明确优先级**：仍需在 macOS 原生客户端把“配置生效 → 对话提议 → 人工授权范围 → 场景实际执行 → 工具/证据展示 → 停止及应用重开”作为一次联合旅程验收，尤其模型实际使用已管理 MCP 与黑盒 Scenario 的装配，不以各自单测替代。通俗作用是确认用户在一个窗口内真正能完成任务，而非继续增加独立底层设施。此前宿主挂起根因未充分证实的风险继续保留，Linux 真机、正式交付与自动更新不在本批范围。未提交、未推送。

### 最新修复：后台读取不能阻塞发送，真实测试仅用 DeepSeek

用户确认今后真实模型验收仅使用 DeepSeek（既定测试模型 `deepseek-flash`），不再调用 Grok。真实桌面模型验收入口在读取安全存储或建立账号连接前检查已配置的模型与官方 DeepSeek 端点；不匹配就拒绝，不自动改正式配置，不回退其他供应商。产品本身的多供应商支持不受影响。

继续检查发现请求调度器的具体活性缺陷：此前允许三个后台读取同时运行，而普通写操作在 running>=3 时不能启动。三个读取卡住会让新消息一直排队，只剩紧急操作预留通道。现限制两个后台读取，分别为普通写操作和紧急停止保留通道；仍保留全局并发与排队上限，不超时释放仍在运行的操作，不重试未知写入。关闭调度器时，在途 IPC 调用也明确拒绝并提示派发结果可能未知，不再等不合作的底层 Promise；底层真实运行名额保持到其实际结束。

验证：调度器/对话桥接/模型桥接 3 文件 **21 项通过**，包含永不返回的后台读取、发送可达、停止可达、关闭后等待结束以及不重复派发。该缺陷可由测试稳定复现并已修复；不把它伪装成对旧实例所有无响应现象的唯一归因。本批未调用真实模型，未消费 Grok 或其他供应商额度。

Desktop TypeScript 构建、验收脚本语法及 `git diff --check` 通过。DeepSeek-only 测试约束同步写入 `AGENTS.md`，用于后续开发；本批未提交或推送。

### 当前增量：当前消息记忆与桌面退出阻塞修复

- 从隔离原生数据库核实此前 `memory_update` 失败：模型提供了虚构的 `id=current` 和非 SHA-256 摘要，得到笼统的 `invalid_history_request`。对应运行时只向历史工具开放 `through-1`，首条用户消息根本无法被检索。现改为包含当前已保存用户消息，排队消息及跨会话内容仍不开放；仍在生成的助手回复不作为原文暴露。
- 搜索/读取返回 `messageSource`（`id/digest/part=message`），记忆可固定仅用户消息的来源。助手回复完成不使这一来源失效；用户消息实际改变仍触发复核。旧引用省略 part 时保留整个对话回合摘要语义，不迁移或改写旧记忆。单条和批量原文读取都支持对应来源，界面原文投影复用同一身份计算。
- 无效记忆输入返回明确的未写入结果与获取真实来源的指引，工具描述明确只有 `status=saved` 才能确认保存。不解析用户自然语言决定执行权限，不新增“普通/复核”聊天模式，也不保证模型永不漏读。
- 退出对照复现：本地 HTTP 请求正文未发完时，原退出会等待活动连接；桌面宿主现显式关闭活动 HTTP 连接。回复服务在 `preClose` 先持久化部分输出为 interrupted 并取消模型，随后才关闭传输；不强制退出进程、不自动重放工具。独立 Server 保留默认连接关闭策略。测试验证旧配置等待、新配置退出，及晚到模型增量不能覆盖退出记录。
- 真实 Grok `responses/grok-4.6` 验收通过：自然语言要求保存当前消息 → 有来源记忆实际落盘 → 助手完成后来源有效 → 下一轮重新读取原文并回答。5 次逻辑模型调用、7 次 HTTP 200；报告 `data/desktop-model-acceptance/current-memory-K5BM2r/report.json`。只发送随机中性数据，不执行目标或命令；不等于重新跑通全部 24 轮真实模型耐久评测。

验证：8 文件 **58 项**记忆/对话/附件/桥接专项通过，退出对照 **2 项**通过；Server 与 Desktop 构建通过，Node 数据库依赖已恢复。未改 renderer，本轮不重跑界面构建。旧遗留实例“连新消息也无响应”的确切根因缺少当时堆栈，不能用这个退出缺陷替代归因；未读 stdout 小型对照未复现阻塞，未据此关闭日志或修改权限。下一步不扩建功能，保持长任务异常恢复与真实模型回读的运行观察，出现复现证据再针对处理。此前的自然语言漏读风险不能被单次探针抹去。

### 当前增量：原生联合验收发现的问题已修正

完成已配置场景的 Skills 创建/停用/Markdown 导入/修订恢复、MCP 显式发现与认证失败恢复，以及真实 Grok 回复期间的队列暂停/修改/调序/恢复。PDF 原生导入、第二页渲染、翻回首页和缩小窗口检查通过；完整退出重开保留会话、附件与配置。详见 `docs/verification/desktop-native-2026-09-16.md` 的 09-17 补充。

本次不是只追加验收：修正了工具失败仍显示“已返回”、PDF.js 现代构建与当前 Electron 不匹配、存储 PDF 类型判断导致第二页被拒，以及原生 Markdown 选择过滤。工具结果先按完整持久输出判断失败再截断展示；旧记录无需重写。记忆失败反馈提供重新读取原文及版本冲突处理指引，不增加特殊聊天模式，不自动扩大权限。PDF 使用配套 legacy 主库/worker，仍按页、只读画布，不执行文档脚本。Impeccable 指导错误提示可理解性，未改变既定白灰对话布局。

完成边界：MCP 原生测试覆盖管理与发现/失败诊断，不声称本次原生模型实际调用 MCP；运行消费继续由既有集成回归证明。Hindsight 借鉴的来源检索与版本记忆已实现，但真实模型自然语言回读可靠性和跨任务经验泛化不能据此记为全部完成。遗留旧宿主挂起根因仍未证实；本次正常退出只能证明本次路径。下一优先级仍是统一对话中长历史记忆的真实模型可靠性及异常恢复验证：实际作用是减少“说记住了却未写入”或漏读关键原文，不扩成另一套助手/调度系统。

本批最终验证：对话/附件/记忆/配置界面专项 9 文件 **76 项通过**；Skills/MCP 实际消费专项 3 文件 **16 项通过**，合计 **92 项**。后者首次受限执行因临时端口监听 EPERM 失败，获准重跑后通过。Server、Desktop TypeScript 与 renderer 生产构建通过；主包 658.86 kB、按需 PDF 块 540.93 kB 的体积提示仍保留。脚本语法检查、`git diff --check` 通过。未执行全量回归，未提交/推送本批变更。
