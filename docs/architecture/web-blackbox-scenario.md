...
更新日期：2026-09-09；当前 Package 版本：0.5.8

0.5.8 声明独立的 `workspace.network` 授权及工具联网地址范围；宿主工作区经当前执行归属检查后使用单独代理出口，HTTP 检查具体 URL，CONNECT 检查站点根/端口并只记录连接层结果。请求前持久记录准入，旧 Run 不增权。代理传输/原生隔离在通用执行包，归属/授权/回执装配在宿主，场景仅声明配置，不拥有直连回退。依赖安装、Node/Python 运行时管理仍未完成；不支持代理的客户端仍不能联网。代理上限独立于场景 HTTP 累计预算。

0.5.7 将 compare/execute 对照扩展为 GET、HEAD、POST、PUT、PATCH、DELETE、OPTIONS；允许在 URL、方法、Session、非凭据请求头或最多 64 KiB 正文中选择一个维度比较，矩阵变体必须维度一致。继续逐请求授权、计费和保存回执，未知结果不重放；没有新增每轮重置，调用者必须考虑重复写请求的实际副作用。字段规则在 Scenario 共享模块中实现，不侵入 Core。调查指导强调完整探索方向和增量成果，未新增调度架构。
...
宿主现在按目标而非标题排除活动/阻塞 Work 的精确重复；模型连续工具循环仍属于通用 Worker。达到回合预算时，宿主先保存停止检查点，再提供一次无工具的有界总结，失败或取消不重发请求。来源过滤后的开放方向与完成/失败摘要作为当前 Run 的共享进度交给三个认知角色；它不是已验证证据或跨 Run 记忆。场景只声明调查方向和方法，不直接调用外部智能体 CLI，也不拥有新的进程/网络权限。
...
0.5.5 增加同一假设内有界 `candidates` 矩阵（最多 16 个顺序变体），绑定预期信号与停止条件，保留旧单 candidate 合同。每个变体重复配对基线，逐请求授权/回执/检查点，未知结果不重发。预算由授权表单声明、运行时从 Scope 读取；累计 HTTP admission 使用 Run 状态，跨调用及子进程重启不清零。默认访问上限仍 64，可明确授权至 512；假设默认 16、最高 128。累计额度不覆盖 Browser 子资源或公开资料工具。通用请示、历史读取和上下文保留以当前源码及[开发状态](../development-status-and-roadmap.md)为准。下述固定数字在 0.5.5 指默认值或未参数化的独立保留上限。
...
`web.hypothesis.register` 把每个候选固定到保留观察和来源库存；`web.validation.execute` 绑定原 Work/计划，最多四次一次性准备后执行两至三轮 HTTP 对照（支持方法及字段见上方 0.5.7 说明）。未知探索效果也会阻止新验证派发。`web.validation.review` 的 supported/refuted 必须同时引用 baseline 和 candidate；差异和审阅意见不会自动产生 verified Finding。
...
- 任意多步业务状态机、每轮重置与任意漏洞利用程序不属于当前有界 HTTP 对照执行器能力；普通正文对照已在 0.5.7 实现，秘密模板仍仅用于准备请求；
- 报告构建器不自动读取或推进 Finding 生命周期，仍需现有图工具与审阅；浏览器/手工请求不自动进入 HTTP 探索库存；
- 真实模型调查效果和 macOS 桌面全链路由后续手动验收确认；Linux 后续单独测试，不能以本机夹具通过替代。

真实效果验收按[手动验收清单](../web-blackbox-manual-acceptance.md)，使用重新构建并审核安装的当前场景包完成桌面调查；当前优先级见[开发状态](../development-status-and-roadmap.md)。保留旧 Run 的原版本绑定，不覆盖旧版本的受信文件或伪造迁移。0.5.1 为各阶段和 Worker pool 声明上下文读取能力，资源授权仅允许包内列出的指导资源；用户资料由宿主配置层继承读取边界，不将场景策略写入底座。
