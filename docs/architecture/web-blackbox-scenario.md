...
真实效果验收按[手动验收清单](../web-blackbox-manual-acceptance.md)，使用重新审核安装的 0.5.8 场景包完成桌面调查；当前开发优先级以开发计划顶部的工具使用闭环为准。保留旧 Run 的原版本绑定，不覆盖旧版本的受信文件或伪造迁移。0.5.1 为各阶段和 Worker pool 声明上下文读取能力，资源授权仅允许包内列出的指导资源；用户资料由宿主配置层继承读取边界，不将场景策略写入底座。
