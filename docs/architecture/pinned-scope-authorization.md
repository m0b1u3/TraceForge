...
2. `compatible_upgrade`：固定到前向 schema/Definition 版本，原 scope/有效期/状态不变，新旧策略函数实现及解析后的权限 Envelope 必须一致。
...
固定的是**宿主已安装、已审核的包版本身份**。当前持久化契约摘要覆盖绑定和 Definition，不是 JS 源码/依赖签名，不证明闭包捕获的数据未改变。
相同函数引用用于保守判断版本兼容，不自动证明两个独立实现语义等价。恶意或随意变更同进程实现仍超出这个契约；不能把版本号当作可执行代码验签或沙箱。
后续受信装配批次已将签名材料和当前使用门禁接入固定策略入口，参见 [场景包审核材料](scenario-package-trust.md)。
文件验签与内存对象来源是两个条件：当前仍须可信宿主明确关联，不能据此宣称自动代码来源证明或闭包隔离。
