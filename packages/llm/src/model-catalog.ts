...
export async function discoverModels(input: LlmEndpointConfig, dependencies: ModelConnectionDependencies = {}): Promise<ModelCatalog> {
  ...
  try {
    ...
    const response = await modelConnectionFetch(config, dependencies)(base + path, { method: "GET", headers, signal: controller.signal });
    if (!response.ok) {
