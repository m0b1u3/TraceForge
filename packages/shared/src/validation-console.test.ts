...
describe("validation console events", () => {
  it("formats the operational validation events and ignores routine telemetry", () => {
    expect(validationTimelineConsoleEvent({ eventType: "validation_task_claimed", detail: "Task=task_1; consensus=insufficient" })).toEqual({ label: "Validation lease", text: "Claimed task_1 · consensus insufficient" });
