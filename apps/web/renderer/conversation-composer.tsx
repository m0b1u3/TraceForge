...
export function ConversationComposer({bridge,conversationId,runs,draft,onChange,onNewMessage,onSettings,disabled,onBusy,inputRef,attachmentReset,sendLabel}:{
  ...
  return <>
    ...
    <form className="composer host-composer" onSubmit={event=>{event.preventDefault();submit();}}>
      ...
      <div className="composer-footnote"><AttachmentPicker selectFiles={bridge.selectAttachments} onError={setError} disabled={disabled||busy||!!pending||invalid} onReading={setReadingAttachments} onAdd={items=>{try{setAttachments(MessageAttachmentsSchema.parse([...attachments,...items]));setError("");}catch{setError("附件最多 4 个，总量超限时请先移除部分附件。");}}}/><button type="button" onClick={onSettings}><GearSix aria-hidden="true"/>模型设置</button><span>{draft.length?`${draft.length} / 16000 · 本地草稿`:"Enter 发送 · Shift+Enter 换行"}</span></div>
    </form>
