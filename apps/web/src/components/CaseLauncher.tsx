  ...
  if (variant === "bar") {
    return (
      <div className="tf-case-bar">
        ...
        <button className="tf-btn tf-btn-icon" disabled={creatingCase || deletingCase} onClick={() => setCreating((v) => !v)}><Plus size={14} weight="bold" /> New</button>
        {caseId && (
          <button className="tf-btn tf-btn-icon tf-btn-danger" disabled={deletingCase || creatingCase} onClick={onDelete} title="Delete case" aria-label="Delete case">
            {deletingCase ? <CircleNotch size={14} className="tf-spin" /> : <Trash size={14} weight="bold" />}
          </button>
        )}
        {creating && (
