...
export function TrafficInspector({ entry }: { entry: TrafficEntry }) {
  ...
  return <div className="context-inspector">
    ...
    <div className="context-inspector-scroll">
      ...
      <dl className="inspector-meta"><div><dt>Observed</dt><dd>{new Date(entry.createdAt).toLocaleString()}</dd></div><div><dt>Content type</dt><dd>{entry.contentType?.split(";")[0] ?? "Unknown"}</dd></div><div><dt>Response size</dt><dd>{entry.responseSize === null || entry.responseSize === undefined ? "Unknown" : `${entry.responseSize.toLocaleString()} B`}</dd></div><div><dt>Request ID</dt><dd><code>{entry.id}</code></dd></div></dl>
      ...
      <section className="inspector-data-block"><header><span>Response headers</span><strong>{responseHeaders.length}</strong></header>{responseHeaders.length ? <dl className="inspector-header-list">{responseHeaders.map(([key, value]) => <div key={key}><dt>{key}</dt><dd>{value}</dd></div>)}</dl> : <p>No response headers captured</p>}</section>
