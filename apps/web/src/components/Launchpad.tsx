...
function FindingSummary({ summary }: { summary: CaseSummary }) {
  ...
  if (!entries.length) return <span className="launchpad-zero">0</span>;
  ...
  return <div className="launchpad-shell">
    ...
    <main className="launchpad-main">
      <section className="launchpad-engagements" aria-labelledby="engagements-heading">
        <header className="launchpad-section-head"><div><span className="section-kicker">Operations</span><h1 id="engagements-heading">Recent engagements</h1><p>Resume an investigation or review captured evidence.</p></div><div className="launchpad-tools"><label><MagnifyingGlass size={15} /><span className="sr-only">Search engagements</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search" /></label><Button variant="outline" size="icon-sm" aria-label="Filter engagements" disabled><FunnelSimple size={15} /></Button></div></header>
        <div className="launchpad-table" role="table" aria-label="Recent engagements">
          <div className="launchpad-table-head" role="row"><span role="columnheader">Engagement</span><span role="columnheader">Status</span><span role="columnheader">Requests</span><span role="columnheader">Findings</span><span role="columnheader">Last activity</span><span aria-hidden="true" /></div>
          <div className="launchpad-table-body">
      ...
      <aside className="launchpad-start" aria-labelledby="new-investigation-heading">
        ...
        <section className="launchpad-readiness"><h3>Environment readiness</h3><button type="button" onClick={() => void openBrowser()}><Browser size={18} /><span><strong>Shared browser</strong><small>Available</small></span><i data-ready="true" /><CaretRight size={14} /></button><button type="button" onClick={() => setSettingsModalOpen(true)}><Robot size={18} /><span><strong>LLM provider</strong><small>{readiness.llm ? "Configured" : "Setup required"}</small></span><i data-ready={readiness.llm} /><CaretRight size={14} /></button><button type="button" onClick={() => void openMcp()}><Wrench size={18} /><span><strong>MCP tools</strong><small>{readiness.mcp} connected</small></span><i data-ready={readiness.mcp > 0} /><CaretRight size={14} /></button></section>
        {last && <button className="launchpad-resume" type="button" onClick={() => void openCase(last.id)} disabled={Boolean(busyId)}><span><small>Resume last session</small><strong>{last.name}</strong><em>{formatRelativeTime(last.lastActivityAt)}</em></span><span>Resume <CaretRight size={14} /></span></button>}
      </aside>
    </main>
  </div>;
}
