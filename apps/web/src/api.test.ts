...
describe("observer warning API helpers", () => {
  it("calls the accept warning endpoint", async () => {
    responses.push(new Response(JSON.stringify({ id: "warn_1", status: "accepted" }), { status: 200 }));

    const warning = await acceptObserverWarning("warn_1");

    expect(calls).toContainEqual(["/api/observer/warnings/warn_1/accept", { method: "POST" }]);
    expect(warning.status).toBe("accepted");
  });

  it("calls the dismiss warning endpoint", async () => {
    responses.push(new Response(JSON.stringify({ id: "warn_1", status: "dismissed" }), { status: 200 }));

    const warning = await dismissObserverWarning("warn_1");

    expect(calls).toContainEqual(["/api/observer/warnings/warn_1/dismiss", { method: "POST" }]);
  ...
  it("throws backend errors for createFact", async () => {
    responses.push(new Response(JSON.stringify({ error: "invalid fact" }), { status: 400 }));
  ...
  it("throws backend errors for createTask", async () => {
    responses.push(new Response(JSON.stringify({ reason: "invalid task" }), { status: 400 }));
