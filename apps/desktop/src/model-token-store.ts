...
export function createModelTokenStore(path: string, cipher: Cipher): OAuthTokenStore {
  ...
  const read = (): Record<string, OAuthTokenRecord> => {
    ...
    if (!data || data.version !== 1 || !data.accounts || typeof data.accounts !== "object" || Array.isArray(data.accounts) || Object.keys(data.accounts).length > 32) throw new Error("Invalid model token store");
  ...
  const save = (accounts: Record<string, OAuthTokenRecord>) => {
    if (!cipher.available()) throw new Error("Secure model storage unavailable");
    if (Object.keys(accounts).length > 32) throw new Error("Too many stored model accounts");
