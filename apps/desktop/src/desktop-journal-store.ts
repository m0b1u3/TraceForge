...
export class DesktopJournalStore {
  ...
  constructor(private file: string) {
    ...
    if (statSync(file).size > 2 * 1048576) throw new Error("Desktop journal capacity exceeded");
    ...
    for (const [key, value] of Object.entries(parsed)) {
      if (!validKey(key) || typeof value !== "string" || value.length > 262144) throw new Error("Invalid desktop journal entry");
  ...
  request(input: unknown): string | null {
    ...
    if (value.operation !== "remove" && value.operation !== "set" || value.operation === "set" && (typeof value.value !== "string" || value.value.length > 262144)
    ...
    if (Object.keys(next).length > 2048 || Buffer.byteLength(text) > 2 * 1048576) throw new Error("Desktop journal capacity exceeded");
