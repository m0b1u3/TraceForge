...
export class DesktopReplyService {
  ...
  constructor(private sql: Database.Database, private provider: () => LlmProvider, private timeoutMs = 240000, private tasks?: ConversationTaskPort) {
  ...
  private present(row: DesktopReply): DesktopReply {
    ...
    return { ...row, ...(taskRequest ? { taskRequest } : {}), contextTruncated: Boolean(row.contextTruncated), reasoningTruncated: Boolean(row.reasoningTruncated),
  ...
  start(conversationId: string, messageId: string) {
    ...
    try { prepared = prepareConversationContext(this.sql, conversationId, current.sequence, model, system); }
    ...
    if(this.active.size){this.update(conversationId,messageId,"","queued");}
  ...
  private drain() {
    ...
    const queued=this.sql.prepare("SELECT conversation_id AS c,message_command_id AS m FROM desktop_replies WHERE state='queued' ORDER BY rowid LIMIT 1").get() as {c:string;m:string}|undefined;
    if(!queued)return;
    try{
      ...
      const prepared=prepareConversationContext(this.sql,queued.c,message.sequence,model,system);
  ...
  private generate(model: LlmProvider, conversationId: string, messageId: string, through: number, prepared: ReturnType<typeof prepareConversationContext>) {
    ...
    void (async () => {
      ...
      const tools = [...conversationHistoryTools, ...(this.tasks?.tools ?? [])];
      ...
      try {
        ...
        for (let turn = 0; turn < 8; turn++) {
          ...
          onTextDelta: delta => {
            ...
            const smaller = prepareConversationContext(this.sql, conversationId, through, model, system, true);
          ...
          history.push({ role: "assistant", content: turnText, toolCalls: result.toolCalls });
          for (const call of result.toolCalls) {
            ...
            let value: unknown;
            try { value = task ? await waitForCancellation(() => this.tasks!.execute(conversationId, messageId, call, abort.signal), abort.signal) : reader.execute(call); }
      ...
      } catch (error) { if (!settled) { try { finish("failed", error instanceof ModelContextOverflowError ? "context_limit" : "provider_failed"); } catch { /* Durable state recovers on restart. */ } } }
