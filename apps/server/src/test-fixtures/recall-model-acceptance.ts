...
export async function runRecallModelAcceptance(provider: Pick<LlmProvider, "extractJson">, options: {
  ...
  try {
    active = await foundationHost({root,objective,modelTimeoutMs:30000,
      foundation:{scenarioPackageRegistry:new ScenarioPackageRegistry([pkg]),toolDiscoverySources:[],
      ...
      model:async args => {
        ...
        if (report.calls.length >= 6) throw new Error("external_call_limit");
        ...
        const signal = AbortSignal.any([stop.signal, args.signal ?? stop.signal, AbortSignal.timeout(30000)]);
        let onAbort!:()=>void;
        ...
        } catch {call.status="failed"; throw new Error("acceptance_model_failed");}
