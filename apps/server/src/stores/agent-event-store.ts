...
export class AgentEventStore {
  ...
  append(caseId: string, kind: AgentEvent["kind"], text: string, tool?: string, occurredAt?: string, refs?: AgentEventRefs): AgentEvent {
    const id = `ae_${randomUUID()}`;
    const createdAt = occurredAt ?? new Date().toISOString();
    const e = AgentEventSchema.parse({ id, caseId, kind, text, tool: tool ?? null, refs: refs ?? null, createdAt });
    this.db.insert(agentEvents).values({
      id, caseId, kind, text, tool: e.tool, refsJson: e.refs ? JSON.stringify(e.refs) : null, createdAt,
    }).run();
    return e;
  }
