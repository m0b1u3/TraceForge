...
`web.validation.execute` durably performs up to four explicit preparation requests, followed by repeated baseline/candidate pairs. Both it and `web.validation.compare` support GET, HEAD, POST, PUT, PATCH, DELETE and OPTIONS, with bounded literal bodies and non-credential headers. Change exactly one request dimension across the matrix: URL, method, Session, headers or body. Within that dimension, keep all unrelated variables fixed. A request body is limited to 64 KiB; credentials remain Host Session handles, not literal headers or bodies. GET/HEAD cannot carry a body.

Repeated requests may have side effects. Use a repeatable test resource within the user's grant, explicitly assess state changes and cleanup, and never infer safety from the HTTP method or response code. The tools do not reset state between rounds. If repeatability requires a reset or an unsupported multi-step sequence, preserve the hypothesis and explain that prerequisite instead of repeatedly mutating a real resource. Preparations happen once and may use authorized Host Session secret handles. Arbitrary browser actions, per-round resets and general exploit programs remain outside these experiment tools. Other exposed tools can contribute observations, but must not be misreported as a completed controlled experiment.
