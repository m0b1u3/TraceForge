...
export async function investigation(action, input, context, capability, dispatch) {
    ...
    let state = loaded.output == null ? { version: 1, candidates: [], active: null } : restore(loaded.output.value);
    const save = async () => {
        if (!write)
            throw new Error("Report cannot change investigation state");
        ...
        revision = receipt.output.revision;
        state = restore(receipt.output.value);
    };
    if (action === "register") {
        ...
        if (existing) {
            if (existing.statement !== statement || JSON.stringify(existing.basisRefs) !== JSON.stringify(basisRefs) || existing.scopeRef !== context.scopeRef
                || (existing.surfaceKey ?? "web.surface.v1") !== inventoryKey)
                throw new Error("Candidate id already binds other material");
            if (!existing.registered)
                throw new Error("Hypothesis registration outcome is unconfirmed; inspect the graph before continuing");
            return candidateResult(existing);
        }
        if (state.candidates.length >= limits.hypotheses)
            throw new Error("Investigation candidate budget exhausted; request additional authorization or report current coverage");
        ...
        if (!Array.isArray(observations))
            throw new Error("Explore authorized surface before registering a hypothesis");
        const known = new Set(observations.flatMap(item => [item.networkReceipt, ...(item.evidenceRefs ?? [])]));
        if (!basisRefs.every(ref => known.has(ref)))
            throw new Error("Hypothesis basis must reference retained surface observations");
        ...
        const candidate = { registered: false, reviewRecorded: false, id, hypothesisId: `web-hypothesis:${sha(`${context.caseId}:${context.runId}:${id}`)}`, statement, basisRefs,
        ...
        state.candidates.find(item => item.id === id).registered = true;
        await save();
        return candidateResult(state.candidates.find(item => item.id === id));
    }
    if (action === "report" || action === "snapshot") {
        exact(input, []);
        const groups = { supportedCandidates: [], refutedCandidates: [], unresolved: [] };
        for (const candidate of state.candidates) {
            ...
            if (candidate.reviewRecorded && candidate.review?.outcome === "supported")
                groups.supportedCandidates.push(row);
            else if (candidate.reviewRecorded && candidate.review?.outcome === "refuted")
                groups.refutedCandidates.push(row);
            else
                groups.unresolved.push(row);
        }
        ...
        const reportRefs = unique([...state.candidates.flatMap(candidate => [...(candidate.registered ? [`knowledge-node:${candidate.hypothesisId}`] : []), ...candidate.basisRefs,
            ...
            ...coverage.inventories.flatMap(item => item.observations.flatMap((observation) => [observation.networkReceipt, ...observation.evidenceRefs]))]);
    ...
    if (!candidate)
        throw new Error("Register the candidate first");
    if (!candidate.registered)
        throw new Error("Hypothesis registration outcome is unconfirmed");
    if (candidate.scopeRef !== context.scopeRef)
        throw new Error("Candidate belongs to another authorization scope");
    if (action === "review") {
        exact(input, ["candidateId", "outcome", "causalMechanism", "expectedBoundary", "securityImpact", "alternatives", "refs"]);
        if (!["observed", "stopped", "reviewed"].includes(candidate.status) && candidate.pending === null)
            throw new Error("Collect observations before review");
        const outcome = requiredText(input.outcome, "Review outcome");
        if (!["supported", "refuted", "inconclusive"].includes(outcome))
            throw new Error("Review outcome is invalid");
        if ((candidate.pending !== null || candidate.status === "stopped") && outcome !== "inconclusive")
            throw new Error("Interrupted or stopped experiments require an inconclusive review");
        if (outcome === "supported" && candidate.assessment !== "repeatable_difference")
            throw new Error("A supported comparison requires a repeatable difference");
        const references = refs(input.refs), known = new Set([...candidate.basisRefs, ...candidate.observations.flatMap(item => item.refs)]);
        if (!references.every(ref => known.has(ref)))
            throw new Error("Review contains unknown evidence references");
        if (outcome !== "inconclusive" && !candidate.observations.some(item => item.refs.some(ref => references.includes(ref))))
            throw new Error("Review must cite experiment observations");
        ...
        if (candidate.review && JSON.stringify(candidate.review) !== JSON.stringify(review))
            throw new Error("Review is immutable; retain conflicting interpretations in the evidence graph");
        if (candidate.review) {
            if (!candidate.reviewRecorded)
                throw new Error("Review persistence outcome is unconfirmed; inspect the graph");
        ...
        candidate.reviewRecorded = true;
        candidate.status = "reviewed";
        // An inconclusive note cannot clear an unknown external effect fence.
        if (candidate.pending === null && state.active === candidate.id)
            state.active = null;
        await save();
    ...
    if (candidate.fingerprint !== null && candidate.fingerprint !== fingerprint)
        throw new Error("Validation plan cannot change during continuation");
    if (candidate.workId !== null && candidate.workId !== workId)
        throw new Error("Resume using the original validation Work");
    if (state.active !== null && state.active !== candidate.id)
        throw new Error("Another validation candidate is active");
    if (candidate.pending !== null || ["observed", "stopped", "reviewed"].includes(candidate.status))
        return candidateResult(candidate);
    ...
    for (const url of unique([...plan.prepare.map(step => step.request.url), plan.baseline.url, ...plan.candidates.map(item => item.url)])) {
        await capability("traceforge.scenario.authorization@1", "authorize_resource", {
    ...
    for (let used = 0; used < budget && candidate.observations.length < sequence.length && Date.now() < deadline; used++) {
        ...
        const observation = { stage: step.stage, status: boundedInteger(body.status, 100, 599, "HTTP status"),
        ...
        observation.refs = unique([...observation.refs, ...receipt.refs]);
        ...
        if (step.expectedStatuses.length && !step.expectedStatuses.includes(observation.status)) {
            ...
            candidate.assessment = "precondition_failed";
        }
        else {
            ...
            if (candidate.observations.length === sequence.length || (plan.stopOn === "repeatable_difference" && observations.length % (plan.rounds * 2) === 0 && latest?.assessment === "repeatable_difference" && signalMatch)) {
                ...
                candidate.assessment = candidate.variantAssessments.some(item => item.assessment === "repeatable_difference") ? "repeatable_difference" : latest?.assessment ?? "insufficient_observations";
            }
        }
        await save();
        candidate = state.candidates.find(item => item.id === candidateId);
...
function handoff(state, inventories) {
...
function parsePlan(value) {
    const plan = plainObject(value, "Validation plan");
    exact(plan, ["prepare", "baseline", "candidate", "candidates", "rounds", "changedCondition", "stopOn", "expectedSignals"]);
    const changedCondition = requiredText(plan.changedCondition, "Controlled change rationale");
    if (!Array.isArray(plan.prepare) || plan.prepare.length > 4)
        throw new Error("At most four preconditions are supported");
    const prepare = plan.prepare.map(value => {
        const step = plainObject(value, "Precondition");
        exact(step, ["request", "expectedStatuses"]);
        if (!Array.isArray(step.expectedStatuses) || step.expectedStatuses.length < 1 || step.expectedStatuses.length > 8)
            throw new Error("Preconditions need bounded expected statuses");
    ...
    for (const candidate of candidates) {
        ...
        if (changed.length !== 1 || changed[0] !== dimension)
            throw new Error("All variants must change exactly the same comparison request dimension");
    }
    const stopOn = plan.stopOn ?? "never", expectedSignals = plan.expectedSignals ?? ["statusChanged", "bodyChanged", "bytesChanged"];
    if (!["never", "repeatable_difference"].includes(stopOn) || !Array.isArray(expectedSignals) || !expectedSignals.length || expectedSignals.length > 3 || expectedSignals.some(v => !["statusChanged", "bodyChanged", "bytesChanged"].includes(v)))
...
function parseRequest(value, prepare) {
    ...
    if (!(prepare ? ["GET", "HEAD", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"] : ["GET", "HEAD"]).includes(method))
        throw new Error("Unsupported workflow method");
    if (prepare)
        requiredText(request.purpose, "Precondition purpose and expected side effects");
    if (!prepare && ["bodyBase64", "secretBody", "captures", "purpose"].some(key => request[key] !== undefined))
        throw new Error("Comparison requests cannot have preparation fields");
    ...
    if (request.bodyBase64 !== undefined && (requiredBase64(request.bodyBase64).length > 87384 || request.secretBody !== undefined))
        throw new Error("Invalid or oversized precondition body");
    if ((request.secretBody !== undefined || request.captures !== undefined) && request.sessionId === undefined)
        throw new Error("Secret templates and captures require a Host Session");
    if (["GET", "HEAD"].includes(method) && (request.secretBody !== undefined || request.bodyBase64))
        throw new Error("GET/HEAD preparations cannot carry a body");
    if (request.secretBody !== undefined) {
        const body = plainObject(request.secretBody, "Secret template");
        exact(body, ["format", "fields"]);
        if (!["form", "json"].includes(body.format))
            throw new Error("Unsupported secret template format");
        const fields = plainObject(body.fields, "Secret fields");
        if (Object.keys(fields).length > 64)
            throw new Error("Too many secret template fields");
        for (const [name, value] of Object.entries(fields)) {
            if (!name || Buffer.byteLength(name) > 256)
                throw new Error("Invalid template field name");
            const selector = plainObject(value, "Template selector");
            exact(selector, ["literal", "secret"]);
            if (Object.keys(selector).length !== 1)
                throw new Error("Choose one template selector");
            if ("secret" in selector)
                requiredText(selector.secret, "Secret handle");
            else if (typeof selector.literal !== "string" || Buffer.byteLength(selector.literal) > 8192 || /(?:password|passwd|token|secret|api.?key)/i.test(name))
                throw new Error("Sensitive fields require a secret handle");
        }
    }
    if (request.captures !== undefined) {
        if (!Array.isArray(request.captures) || request.captures.length > 16)
            throw new Error("Invalid capture list");
        for (const value of request.captures) {
            const capture = plainObject(value, "Capture");
            exact(capture, ["name", "start", "end", "maximumBytes"]);
            if (!/^[a-zA-Z][a-zA-Z0-9_.:-]{0,127}$/.test(requiredText(capture.name, "Capture name")))
                throw new Error("Invalid capture name");
            for (const delimiter of [capture.start, capture.end])
                if (Buffer.byteLength(requiredText(delimiter, "Capture delimiter")) > 256)
                    throw new Error("Capture delimiter too large");
...
function assess(observations) {
    if (observations.some(item => item.truncated))
        return "truncated_observations";
    const signature = (item) => JSON.stringify([item.status, item.bytes, item.digest]);
    ...
    if (new Set(baseline.map(signature)).size !== 1 || new Set(candidate.map(signature)).size !== 1)
...
function refs(value) {
    if (!Array.isArray(value) || value.length < 1 || value.length > 32)
        throw new Error("Evidence references are required and bounded");
...
function restore(value) {
    ...
    if (state.version !== 1 || !Array.isArray(state.candidates) || state.candidates.length > 128
        || (state.active !== null && !state.candidates.some(item => item.id === state.active)))
    ...
    for (const candidate of state.candidates) {
        if (!Array.isArray(candidate.observations) || candidate.observations.length > 100
            || !["queued", "running", "observed", "stopped", "reviewed"].includes(candidate.status))
            throw new Error("Invalid candidate checkpoint");
    }
    return structuredClone(state);
}
async function recordCandidate(candidate, capability) {
