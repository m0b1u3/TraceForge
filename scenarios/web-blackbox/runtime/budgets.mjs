...
export async function budgets(capability) {
...
export async function reserveRequest(capability, identity) {
    ...
    const revision = loaded.output?.revision ?? 0, used = loaded.output?.value ?? { version: 1, ids: [] };
    if (used.version !== 1 || !Array.isArray(used.ids) || used.ids.length > 4096 || used.ids.some((v) => typeof v !== "string"))
        throw new Error("Invalid HTTP budget ledger");
    const id = sha(identity);
    if (used.ids.includes(id))
        return;
