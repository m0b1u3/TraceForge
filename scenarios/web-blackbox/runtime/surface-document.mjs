...
export function inspectDocument(body, base, origins, maximum) {
    const clean = body.replace(/<!--[\s\S]*?-->/g, " ")
        .replace(/<(script|style|textarea)\b[^>]*>[\s\S]*?<\/\1\s*>/gi, " ");
    const sameOrigin = [], external = [], forms = [];
    let form;
    ...
    for (const match of clean.matchAll(/<\/?[a-z][^>]*>/gi)) {
        ...
        if (form && ["input", "select", "button"].includes(name) && attrs.name) {
            if (form.fields.length >= 16)
                form.fieldsTruncated = true;
            else
                form.fields.push({ name: attrs.name.slice(0, 128), type: (attrs.type ?? name).slice(0, 32) });
        }
    ...
    while (Buffer.byteLength(JSON.stringify(forms)) > 4096) {
        forms.pop();
        truncated = true;
    }
...
function attributes(tag) {
    ...
    for (const match of tag.matchAll(/\s([a-z_:][a-z0-9_.:-]*)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/gi)) {
        ...
        if (!(key in values))
            values[key] = decode(match[2] ?? match[3] ?? match[4] ?? "");
    }
    return values;
}
...
function httpUrl(value, base) {
    try {
        return canonicalHttpUrl(new URL(value, base).href, "Document URL");
    }
    catch {
        return null;
    }
