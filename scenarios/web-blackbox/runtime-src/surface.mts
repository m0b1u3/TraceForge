...
type CapabilityCall = (capability: string, action: string, input: unknown, suffix: string) => Promise<CapabilityReceipt>;
interface SurfaceState { schemaVersion: 1; seeds: string[]; queue: string[]; visited: string[]; observations: JsonObject[]; skipped: JsonObject[]; pending: string | null;
  omissions: { queuedUrls: number; observations: number; skipped: number; documentHints: number; legacyUnknown: boolean } }

export async function exploreSurface(input: JsonObject, capability: CapabilityCall): Promise<ToolResult> {
  ...
  if (state.pending !== null) return succeeded("Surface request outcome is unconfirmed; automatic replay is blocked", {
  ...
  if (combinedSeeds.length > 16) throw new Error("Surface seed capacity exhausted; preserve existing coverage and report the limit");
  state.seeds = combinedSeeds;
  const queue = unique([...state.queue, ...seeds]).filter((value) => !state.visited.includes(value));
  state.omissions.queuedUrls += Math.max(0, queue.length - 32); state.queue = queue.slice(0, 32);
  if (!state.queue.length && !state.visited.length) throw new Error("At least one Surface seed is required for a new exploration");
  const invocationObservations: JsonObject[] = [], refs: string[] = [];
  let step = 0;
  while (step < maximum && state.queue.length && state.visited.length < limits.urls) {
    const url = state.queue.shift()!;
    if (state.visited.includes(url)) continue;
    const key = sha(url);
    let authorization: CapabilityReceipt;
    try {
      authorization = await capability("traceforge.scenario.authorization@1", "authorize_resource",
        { action: "web.request.replay", resourceKind: "network.url", value: url }, `surface-authorization:${key}`);
    } catch {
      state.visited.push(url); state.skipped.push({ url, reason: "authorization_not_confirmed" });
      state.omissions.skipped += Math.max(0, state.skipped.length - 16); state.skipped = state.skipped.slice(-16);
      ({ state, revision } = await saveSurface(capability, state, revision, step, stateKey)); step += 1; continue;
    }
    state.pending = url;
    ...
    }, `surface-http:${key}`).catch(async error=>{if(error instanceof BudgetExhausted){state.pending=null;state.queue.unshift(url);({state,revision}=await saveSurface(capability,state,revision,step,stateKey));}throw error;});
    ...
    state.visited.push(url); state.observations.push(saved); state.visited = unique(state.visited);
    state.omissions.observations += Math.max(0, state.observations.length - 16); state.observations = state.observations.slice(-16);
    state.pending = null;
    invocationObservations.push(saved); refs.push(artifact.contentRef, ...artifactReceipt.refs, ...evidenceReceipt.refs, ...execution.refs);
    ({ state, revision } = await saveSurface(capability, state, revision, step, stateKey)); step += 1;
  }
  ({ state, revision } = await saveSurface(capability, state, revision, step, stateKey));
...
async function saveSurface(capability: CapabilityCall, state: SurfaceState, revision: number, step: number, stateKey: string) {
  while (Buffer.byteLength(JSON.stringify(state)) > 192 * 1024 && state.observations.length) {
    state.observations.shift(); state.omissions.observations += 1;
  }
  if (Buffer.byteLength(JSON.stringify(state)) > 192 * 1024) throw new Error("Surface checkpoint capacity exhausted");
  ...
  return { state: restoreSurfaceState(receipt.output), revision: receipt.output.revision as number };
}

function restoreSurfaceState(record: any): SurfaceState {
  if (record === null || record === undefined) return { schemaVersion: 1, seeds: [], queue: [], visited: [], observations: [], skipped: [], pending: null,
  ...
  return { schemaVersion: 1, seeds: value.seeds.map((item: unknown) => canonicalHttpUrl(item, "Saved seed")).slice(0, 16),
    queue: value.queue.map((item: unknown) => canonicalHttpUrl(item, "Saved queued URL")).slice(0, 32),
    visited: value.visited.map((item: unknown) => canonicalHttpUrl(item, "Saved visited URL")).slice(0, 512),
...
export function discoverLinks(body: string, base: string, origins: Set<string>, maximum: number) {
...
function decodeBody(value: unknown): string {
  ...
  if (Buffer.from(encoded, "base64").toString("base64") !== encoded) throw new Error("Surface response body is invalid");
...
function textSnippet(value: string, maximum: number): string {
