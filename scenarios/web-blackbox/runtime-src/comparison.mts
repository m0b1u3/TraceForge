...
export async function compareHttp(input: JsonObject, capability: Capability,
  ...
  const output=(status:"interrupted"|"complete"|"in_progress")=>result(status,state,rounds,dimensions[0]!,candidates.length,expectedSignals);
  if (state.pending !== null) return output("interrupted");
  if(state.stopped)return output("complete");
  const save = async () => {
  ...
  for (let used = 0; used < maxRequests && state.observations.length < planned && Date.now()<deadline; used++) {
    ...
    if(stopOn==="repeatable_difference"&&state.observations.length%(rounds*2)===0){
      ...
      if(value.assessment==="repeatable_difference"&&value.pairs.every((pair:JsonObject)=>expectedSignals.some(signal=>pair[signal]===true)))state.stopped=true;
    }
    await save();
...
function result(status: "interrupted" | "complete" | "in_progress", state: ComparisonState, rounds: number, dimension: string, variantCount=1, expectedSignals:string[]=[]): ToolResult {
