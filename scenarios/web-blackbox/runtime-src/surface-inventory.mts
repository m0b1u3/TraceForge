...
export const surfaceKey = (sessionId?: string | null) => sessionId ? `web.surface.v1:${sha(sessionId).slice(0, 16)}` : "web.surface.v1";

export async function registerInventory(capability: Capability, sessionId: string | null) {
  ...
  if (entries.length >= 16) throw new Error("Surface inventory limit reached; report coverage instead of replacing identities");
...
export async function readInventories(capability: Capability, candidateKeys: string[] = []) {
  const loaded = await capability("traceforge.scenario.state@1", "read", { operation: "read", key: catalogKey }, "inventory-read");
  const entries = catalog(loaded.output?.value);
  const keys = unique(["web.surface.v1", ...entries.map(item => item.key), ...candidateKeys]);
  const inventories: JsonObject[] = [];
  for (const key of keys) {
    if (!/^web\.surface\.v1(?::[a-f0-9]{16})?$/.test(key)) throw new Error("Invalid surface inventory key");
    ...
    if (state.schemaVersion !== 1 || ![state.visited, state.queue, state.observations, state.skipped].every(Array.isArray)) throw new Error("Invalid surface inventory checkpoint");
    inventories.push({ key, sessionId: entries.find(item => item.key === key)?.sessionId ?? null,
      ...
      observations: state.observations.map((item: JsonObject) => ({ url: item.url, status: item.status, contentType: item.contentType,
...
function catalog(value: unknown): Array<{ key: string; sessionId: string | null }> {
  if (value == null) return [];
  const state = plainObject(value, "Surface catalog");
  if (state.version !== 1 || !Array.isArray(state.entries) || state.entries.length > 16) throw new Error("Invalid surface catalog");
  return state.entries.map(item => {
    ...
    if (entry.key !== surfaceKey(sessionId)) throw new Error("Invalid inventory binding");
