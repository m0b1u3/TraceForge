...
description: 桌面对话宿主视觉记录，附独立预览的历史设计参考
colors:
  ink: "#202329"
  muted: "#626875"
  line: "#d9dce2"
...
当前宿主入口：`apps/web/renderer/index.html` → `main.tsx` → `host-workbench.tsx`（存在桌面对话桥接时），宿主样式为 `host-workbench.css`。`workbench.tsx`、`preview-state.ts` 是无桥接时的独立演示，不是当前宿主功能清单。当前交互以 `docs/desktop-renderer-surface-brief.md` 与实现为准，历史演示说明见 `docs/desktop-renderer-preview.md`。
...
此处旧版“消息用途”下拉框及“不执行工具”的助手模式描述已移除。当前统一输入由同一助手工具循环处理历史读取、记忆和任务端口；任务执行仍需既有授权，不因对话或历史内容自动获得权限。没有额外的手动原文复核模式。
